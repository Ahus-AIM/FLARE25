...
class TwoWayAttentionBlock3D(nn.Module):
    ...
    def forward(self, queries: Tensor, keys: Tensor, query_pe: Tensor, key_pe: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        q = queries + query_pe
        k = keys + key_pe
        queries = self.cross_attn_token_to_image(q=q, k=k, v=keys, residual_stream=queries)
        ...
        q = queries + query_pe
        k = keys + key_pe
        keys = self.cross_attn_image_to_token(q=k, k=q, v=queries, residual_stream=keys)
...
class NormalizedMaskDecoder3D(nn.Module):
    ...
    def normalize_weights(self):
        # self.transformer.normalize_weights()
