...
def random_interact(
    ...
    """
    Get clicks using random sampling from the error mask.
    ...
    try:
        for i in range(batch_size):
            ...
            if error_mask.sum() > 0:
                # Randomly sample a point from the error mask
                coords = np.argwhere(error_mask.cpu().numpy())
                center = tuple(coords[np.random.choice(coords.shape[0])])
        ...
        print(f"Error in random_interact function. Returning empty points lists.\n{e}")
        batch_points, batch_labels = [], []
...
def compute_largest_error_point(error_mask: torch.Tensor) -> tuple[int, int, int]:
    if error_mask.device.type == "cuda":
        ...
        cuda_index = error_mask.device.index

        # Set the correct CUDA device for CuPy
