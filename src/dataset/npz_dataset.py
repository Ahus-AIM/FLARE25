...
class NPZDataset(Dataset):
    ...
    def _gather_data(self) -> (List[str], List[str]):
        """Recursively collects all files with the specified suffix in base_dir and extracts modality info."""
        file_paths: List[str] = []
        modalities: List[str] = []
        for root, _, files in os.walk(self.base_dir):
            for file in files:
                ...
                # Assume modality is the folder immediately under base_dir.
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # Select a label from non-background (assumed >0) labels
        ...
        labeldata = torch.tensor(data["gts"] == selected_label)
        imgdata = torch.tensor(data["imgs"])
        stacked_data = torch.stack([labeldata, imgdata], dim=0)

        while (stacked_data.shape[1] * stacked_data.shape[2] * stacked_data.shape[3]) > self.size_threshold:
            min_dim_index = int(np.argmin(stacked_data.shape[1:]))
            kernel_size = [2, 2, 2]
            kernel_size[min_dim_index] = 1
            stacked_data = (
        ...
        return {
            ...
            "spacing": torch.tensor(data["spacing"]),
            "rel_path": rel_path,
        }
...
def create_weighted_dataset_folder_sampler(dataset: NPZDataset) -> WeightedRandomSampler:
    """
    ...
    for file_path in dataset.file_paths:
        rel_path = os.path.relpath(file_path, dataset.base_dir)
